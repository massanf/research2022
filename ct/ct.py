...
class ctset:
    def __init__(self, name: str, type):
        ...
        for idx, file in enumerate(
            glob.glob(str(here / "data" / f"{self.name}" / "ct" / "*"))
        ):
        ...
        self.raw_data = [
            datum for datum in self.raw_data if hasattr(datum, "ImagePositionPatient")
        ]
        ...
        self.raw_data = sorted(self.raw_data, key=lambda x: x.ImagePositionPatient[2])
    ...
    def get_volume(self, zm=1, pad=0):
        ...
        volume = zoom(cp.asnumpy(volume), (zm, zm, zm), order=0, mode="nearest")
