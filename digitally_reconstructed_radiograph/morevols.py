...
for vol in range (0, volnum):
    ...
    for transformangle in range (0, 2):
        for transformgain in range (0, gainnum + 1):
            ...
            print(newvolname)
            (script_dir / Path('../raw_dcm') / Path(newvolname)).mkdir(parents=True, exist_ok=True)
            
            for i in range (1, 401):
                ds = pydicom.dcmread(script_dir / Path('../original_raw_dcm') / Path(originvolname) / Path('1-{:03d}.dcm'.format(i)))
                data = ds.pixel_array
                ...
                # transformation
                ratio = .9 + (transformgain /(gainnum * 5.))
                if transformangle == 0:
                    data = cv2.resize(data, dsize=(data.shape[1], int(data.shape[0] * ratio))) # change width
                    ...
                    data = cv2.resize(data, dsize=(int(data.shape[1] * ratio), data.shape[0]))
                ...
                ds.save_as(script_dir / Path('../raw_dcm') / Path(newvolname) /Path('1-{:03d}.dcm'.format(i)))
