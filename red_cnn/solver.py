...
class Solver(object):
    ...
    def denormalize_(self, image):
        image = (
            image * (self.norm_range_max - self.norm_range_min) + self.norm_range_min
        )
        return image
        ...
        for epoch in tqdm(range(1, self.num_epochs), leave=False):
            ...
            for iter_, (x, y) in enumerate(tqdm(self.data_loader, leave=False)):
                ...
                if total_iters % self.print_iters == 0:
                    ...
                    with open(logpath, "a") as f:
                        f.write(
                            f"{total_iters},{epoch},{self.num_epochs},{iter_+1},{len(self.data_loader)},{loss.item():.8f}\n"
                        )
        ...
        with torch.no_grad():
            with tqdm(total=len(self.data_loader), desc=self.mode) as pbar:
                for i, (x, y) in enumerate(self.data_loader):
                    ...
                    pred = self.trunc(
                        self.denormalize_(pred.view(shape_, shape_).cpu().detach())
                    )
