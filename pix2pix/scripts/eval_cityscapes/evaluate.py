...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Path to the generated images to be evaluated",
)
parser.add_argument(
    "--output_dir", type=str, required=True, help="Where to save the evaluation results"
)
parser.add_argument(
    "--caffemodel_dir",
    type=str,
    default="./scripts/eval_cityscapes/caffemodel/",
    help="Where the FCN-8s caffemodel stored",
)
parser.add_argument("--gpu_id", type=int, default=0, help="Which gpu id to use")
parser.add_argument(
    "--split", type=str, default="val", help="Data split to be evaluated"
)
parser.add_argument(
    "--save_output_images",
    type=int,
    default=0,
    help="Whether to save the FCN output images",
)
...
def main():
    ...
    caffe.set_mode_gpu()
    net = caffe.Net(
        args.caffemodel_dir + "/deploy.prototxt",
